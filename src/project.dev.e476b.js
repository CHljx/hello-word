__require = function e(t, n, r) {
  ...
  HttpHelper: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "f069dKJzelKwYRwSlKCLR8X", "HttpHelper");
    "use strict";
    var HttpHelper = {
  ...
  card: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "2497ak/hK1IEpLzQ7r93wUQ", "card");
    "use strict";
    var mvsApp = require("./matchvs");
    cc.Class({
      ...
      onLoad: function onLoad() {
        var _self = this;
        _self._down = false;
        _self.id = new Date().getTime();
        _self.attackMAp = {};
        _self.bindEvent();
      },
      bindEvent: function bindEvent() {
        var _self = this;
        _self.node.on(cc.Node.EventType.TOUCH_START, function(event) {
          if (1 != _self.status) return;
          if (_self.scene.enegy < _self.enegy) return;
          _self.positionBackup = {
            x: this.node.x,
            y: this.node.y
          };
          _self.rotationBackup = _self.node.rotation;
          _self.node.opacity = 160;
          _self.node.scale = .3;
          _self.node.rotation = 0;
          _self._down = true;
          _self.startMove();
          event.stopPropagation();
        }, _self);
        _self.node.on(cc.Node.EventType.TOUCH_MOVE, function(event) {
          if (1 != _self.status) return;
          if (!_self._down) {
            event.stopPropagation();
            return;
          ...
          _self.node.x += delta.x;
          _self.node.y += delta.y;
          event.stopPropagation();
        }, this);
        _self.node.on(cc.Node.EventType.TOUCH_END, function(event) {
          if (1 != _self.status) return;
          if (!_self._down) {
            event.stopPropagation();
            return;
        ...
        _self.node.on(cc.Node.EventType.TOUCH_CANCEL, function(event) {
          if (1 != _self.status) return;
          if (!_self._down) {
            event.stopPropagation();
            return;
      ...
      startMove: function startMove() {
        var _self = this;
        _self.scene.placeBox.node.active = true;
      },
      endMove: function endMove() {
        var _self = this;
        if (_self.placeTarget) {
          var worldPosition = _self.placeTarget.node.convertToWorldSpaceAR(cc.v2(0, 0));
          _self.scene.addSelfRole(_self.scene.node.convertToNodeSpaceAR(worldPosition), _self.classify);
          _self.scene.nextCard(_self.index);
          _self.scene.reduceEnegy(_self.enegy);
          mvsApp.send({
            type: "card",
            info: {
              position: _self.scene.node.convertToNodeSpaceAR(worldPosition),
              classify: _self.classify
            }
          });
          _self.scene.placeBox.node.active = false;
          _self.node.destroy();
        } else {
          _self.node.x = _self.positionBackup.x;
          _self.node.y = _self.positionBackup.y;
          _self.node.scale = 1;
          _self.node.rotation = _self.rotationBackup;
          _self.node.opacity = 255;
          _self._down = false;
          _self.scene.placeBox.node.active = false;
        }
      },
      setOpacity: function setOpacity() {
        ...
        _self.scene.enegy > _self.enegy ? _self.node.opacity = 255 : _self.node.opacity = 160;
      },
      ...
      onCollisionStay: function onCollisionStay(other, self) {
        var _self = this, attackCard = other.getComponent("card");
        other.getComponent("place") && (_self.placeTarget = other);
      },
      onCollisionExit: function onCollisionExit(other, self) {
        var _self = this;
        _self.placeTarget = null;
      }
    });
    cc._RF.pop();
  }, {
  ...
  game: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "9bf68IAcu5INZg2RhotMKaA", "game");
    "use strict";
    var mvsApp = require("./matchvs");
    cc.Class({
      extends: cc.Component,
      ...
      onLoad: function onLoad() {
        ...
        _self.cards = [];
        _self.roles = [];
        mvsApp.hasEnterGame = 1;
        _self.nextBox.node.zIndex = 1;
        _self.winLayout.node.zIndex = 2;
        _self.loseLayout.node.zIndex = 2;
        _self.type = Math.floor(Math.random());
        _self.nextPos = {
          x: -267,
          y: -515
        };
        ...
        _self.enableCollision();
        _self.bindEvent();
        _self.avatarMy.spriteFrame = new cc.SpriteFrame(cc.loader.getRes(mvsApp.userInfo.avatar));
        _self.nameMy.string = mvsApp.userInfo.name;
        _self.roomLabel.string = mvsApp.roomInfo.roomID;
        mvsApp.onEvent("notify", function(eventInfo) {
          _self.notify(eventInfo);
        });
        mvsApp.rsp.leaveRoomNotify = this.leaveRoomNotify.bind(this);
        if (mvsApp.roomOwner) {
          _self.btnStart.node.opacity = 160;
          _self.btnReady.node.active = false;
        } else {
          _self.btnStart.node.active = false;
          mvsApp.engine.sendEvent(JSON.stringify({
            type: "joinRoom",
            userInfo: mvsApp.userInfo
          }));
        }
      },
      bindEvent: function bindEvent() {
        var _self = this;
        _self.btnReady.node.on("click", _self.readySend.bind(_self));
        _self.btnStart.node.on("click", function() {
          _self.startSend();
          _self.startGame();
        });
      },
      readySend: function readySend() {
        var _self = this;
        if (1 == _self.status) return;
        _self.status = 1;
        _self.btnReady.node.opacity = 160;
        mvsApp.send({
          type: "ready"
        });
      },
      ...
      notify: function notify(eventInfo) {
        var _self = this, msg = JSON.parse(eventInfo.cpProto);
        console.log(eventInfo);
        switch (msg.type) {
         case "joinRoom":
          _self.joinRoomNotify(msg.userInfo, msg.gameType);
          break;
         ...
         case "card":
          _self.cardNotify(msg.info);
          break;

         case "result":
          _self.resultNotify(msg.msg);
        }
      },
      readyNotify: function readyNotify() {
        var _self = this;
        _self.btnStart.node.opacity = 255;
        _self.status = 1;
      },
      startNotify: function startNotify() {
        var _self = this;
        _self.status = 1;
        _self.startGame();
      },
      cardNotify: function cardNotify(role) {
        var _self = this;
        _self.addAttackRole(role.position, role.classify);
      },
      resultNotify: function resultNotify(msg) {
        var _self = this;
        "win" == msg ? _self.win(true) : _self.lose(true);
      },
      joinRoomNotify: function joinRoomNotify(userInfo, gameType) {
        var _self = this;
        console.log("joinRoomNotify", userInfo, gameType);
        _self.nameAtack.string = userInfo.name;
        cc.loader.load({
          url: userInfo.avatar,
          type: "jpg"
        }, function(err, texture) {
          _self.avatarAtack.spriteFrame = new cc.SpriteFrame(texture);
        });
        void 0 !== gameType && (_self.type = !gameType);
        mvsApp.roomOwner && mvsApp.engine.sendEvent(JSON.stringify({
          type: "joinRoom",
          userInfo: mvsApp.userInfo,
          gameType: _self.type
        }));
      ...
      startGame: function startGame() {
        var _self = this, card1 = null, card2 = null, card3 = null;
        if (0 == _self.status) return;
        _self.status = 2;
        _self.btns.node.active = false;
        _self.addCard(0, cc.instantiate(_self.getRandomCard()));
        _self.addCard(1, cc.instantiate(_self.getRandomCard()));
        _self.addCard(2, cc.instantiate(_self.getRandomCard()));
        _self.addNext();
        _self.engenTimer = 0;
      },
      refreshEnegy: function refreshEnegy() {
        var _self = this;
        _self.engenTimer = 0;
        _self.enegy > 10 && (_self.enegy = 10);
        _self.enegy < 0 && (_self.enegy = 0);
        _self.enegyBar.progress = _self.enegy / 10;
        _self.enegyLabel.string = _self.enegy;
      },
      addEnegy: function addEnegy() {
        var _self = this;
        _self.enegy++;
        _self.refreshEnegy();
      },
      reduceEnegy: function reduceEnegy(val) {
        var _self = this;
        _self.engenTimer = 0;
        _self.enegy -= val;
        _self.refreshEnegy();
      },
      addAttackRole: function addAttackRole(position, type) {
        var _self = this, role = role = cc.instantiate({
          1: _self.role1,
          2: _self.role2,
          3: _self.role3,
          4: _self.role4,
          5: _self.role5,
          6: _self.role6
        }[type]), roleScript = role.getComponent("role");
        _self.node.addChild(role);
        role.setPosition({
          x: position.x,
          y: position.y + 720
        });
        role.scene = _self;
        roleScript.relation = "attack";
      },
      addSelfRole: function addSelfRole(position, type) {
        var _self = this, role = role = cc.instantiate({
          1: _self.role1,
          2: _self.role2,
          3: _self.role3,
      ...
      getRandomCard: function getRandomCard() {
        var _self = this;
        return 0 == _self.type ? [ _self.card1, _self.card2, _self.card3 ][Math.floor(3 * Math.random())] : [ _self.card4, _self.card5, _self.card6 ][Math.floor(3 * Math.random())];
      },
      addCard: function addCard(index, component, hasAdd) {
        var _self = this;
        var card = component.getComponent("card");
        !hasAdd && _self.node.addChild(component);
        component.setPosition(_self.preparePos[index]);
        component.rotation = _self.preparePos[index].rotation;
        component.scale = 1;
        card.scene = _self;
        card.index = index;
        card.status = 1;
      ...
      addNext: function addNext() {
        var _self = this;
        _self.cardNext = cc.instantiate(_self.getRandomCard());
        _self.node.addChild(_self.cardNext);
        _self.cardNext.setPosition(_self.nextPos);
        _self.cardNext.scale = 90 / 157.5;
        _self.cardNext.getComponent("card").status = 0;
      },
      nextCard: function nextCard(index) {
        var _self = this;
        _self.addCard(index, _self.cardNext, true);
        _self.addNext();
      },
      enableCollision: function enableCollision() {
        var _self = this;
      ...
      update: function update(dt) {
        ...
        if (2 == _self.status) {
          _self.engenTimer += dt;
          1 == _self.type && _self.engenTimer >= 1 && _self.addEnegy();
          0 == _self.type && _self.engenTimer >= 1.3 && _self.addEnegy();
        }
      },
      leaveRoom: function leaveRoom() {
        var _self = this;
        mvsApp.engine.leaveRoom();
        cc.director.loadScene("index");
      },
      again: function again() {
        var _self = this;
        _self.winLayout.node.active = false;
        _self.loseLayout.node.active = false;
        _self.btns.node.active = true;
        _self.status = 0;
        _self.initTown();
        if (mvsApp.roomOwner) {
          _self.btnStart.node.opacity = 160;
          _self.btnReady.node.active = false;
          _self.btnStart.node.active = true;
        } else {
          _self.btnReady.node.opacity = 255;
          _self.btnStart.node.active = false;
          _self.btnReady.node.active = true;
        }
      },
      initTown: function initTown() {
        var _self = this, townSelf = _self.townSelf.getComponent("town"), townAtack = _self.townAtack.getComponent("town");
        townSelf.life = 100;
        townSelf.label.string = 100;
        townSelf.lifeBar.progress = 1;
        townAtack.life = 100;
        townAtack.label.string = 100;
        townAtack.lifeBar.progress = 1;
      },
      win: function win(triggerByNotify) {
        var _self = this, children = _self.node.children;
        _self.status = 0;
        for (var index = children.length - 1; index >= 0; index--) {
          children[index].getComponent("card") && children[index].destroy();
          children[index].getComponent("role") && children[index].destroy();
        }
        _self.winLayout.node.active = true;
        triggerByNotify || mvsApp.send({
          type: "result",
          msg: "fail"
        });
      },
      lose: function lose(triggerByNotify) {
        var _self = this, children = _self.node.children;
        _self.status = 0;
        for (var index = children.length - 1; index >= 0; index--) {
          children[index].getComponent("card") && children[index].destroy();
        ...
        _self.loseLayout.node.active = true;
        triggerByNotify || mvsApp.send({
          type: "result",
          msg: "win"
        });
      ...
      leaveRoomNotify: function leaveRoomNotify() {
        var _self = this;
        _self.nameAtack.string = "";
        _self.avatarAtack.spriteFrame = null;
        mvsApp.roomOwner || (mvsApp.roomOwner = 1);
      }
    });
    cc._RF.pop();
  }, {
  ...
  index: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "956d0+rw3dM65AVaJMVf5or", "index");
    "use strict";
    var mvsApp = require("./matchvs");
    ...
    cc.Class({
      ...
      properties: {
        ...
        btnInputRoom: cc.Button,
        loadBar: cc.Node,
        inputPanel: cc.Layout,
        aboutPanel: cc.Layout,
        helpPanel: cc.Layout,
        infoPanel: cc.Layout,
        mask: cc.Layout,
      ...
      onLoad: function onLoad() {
        var _self = this;
        _self.roomString = [];
        _self.userAvatar.spriteFrame = new cc.SpriteFrame(cc.loader.getRes(mvsApp.userInfo.avatar));
        _self.userName.string = mvsApp.userInfo.name;
        _self.bindEvent();
        _self.MatchvsInit();
      },
      bindEvent: function bindEvent() {
        var _self = this;
        _self.btnCreateRoom.node.on("click", _self.createRoom.bind(_self));
        _self.btnJoinRoom.node.on("click", _self.joinRoom.bind(_self));
        _self.btnInputRoom.node.on("click", _self.inputRoom.bind(_self));
      },
      MatchvsInit: function MatchvsInit() {
        var _self = this;
        mvsApp.rsp.createRoomResponse = this.createRoomResponse.bind(this);
        mvsApp.rsp.joinRoomResponse = this.joinRoomResponse.bind(this);
      },
      createRoomResponse: function createRoomResponse(rsp) {
        var _self = this;
        if (200 == rsp.status) HttpHelper.get("https://ggz.lejugame.com/api/create_game_cloud_room.php?cloud_room_number=" + rsp.roomID, function(oRep) {
          rsp.roomID = oRep.data.room_id;
          mvsApp.roomInfo = rsp;
          mvsApp.roomOwner = 1;
      ...
      joinRoomResponse: function joinRoomResponse(status, roomUserInfoList, roomInfo) {
        var _self = this, statusMsg = "\u623f\u95f4\u52a0\u5165\u5931\u8d25:\u5185\u90e8\u670d\u52a1\u5668\u9519\u8bef";
        if (200 == status) {
          roomInfo.roomID = _self.roomString.join("");
          mvsApp.roomInfo = roomInfo;
          mvsApp.roomOwner = 0;
          cc.director.loadScene("game");
        } else {
          ...
          _self.labelJoin.string = statusMsg;
          _self.hideLoad();
        }
      },
      inputRoom: function inputRoom() {
        var _self = this;
        _self.labelJoin.string = "";
        _self.roomString = [];
        _self.doUpdateRoomString();
        _self.showPanel("input");
      },
      joinRoom: function joinRoom() {
        var _self = this;
        if (!_self.roomString.join("").match(/\d{4}/g)) {
          _self.labelJoin.string = "\u8bf7\u8f93\u51654\u4f4d\u6570\u623f\u95f4\u53f7";
          return;
        }
        _self.load();
        HttpHelper.get("https://ggz.lejugame.com/api/get_cloud_room_info.php?room_id=" + _self.roomString.join(""), function(oRep) {
          if (0 == oRep.code) {
            _self.labelJoin.string = "\u672a\u627e\u5230\u8be5\u623f\u95f4";
            _self.hideLoad();
            return;
          }
          if (!oRep.data.game_cloud_room_number) {
            _self.labelJoin.string = "\u672a\u627e\u5230\u8be5\u623f\u95f4";
            _self.hideLoad();
            return;
          }
          mvsApp.engine.joinRoom(oRep.data.game_cloud_room_number, mvsApp.userInfo);
        });
      },
      about: function about() {
        var _self = this;
        _self.aboutPanel.node.active = true;
        _self.mask.node.active = true;
      },
      load: function load() {
        var _self = this;
        _self.loadBar.active = true;
      },
      hideLoad: function hideLoad() {
        var _self = this;
        _self.loadBar.active = false;
      },
      showPanel: function showPanel(type) {
        var _self = this;
        2 == arguments.length && (type = arguments[1]);
        _self[type + "Panel"].node.active = true;
        _self.mask.node.active = true;
      },
      hidePanel: function hidePanel() {
        var _self = this;
        _self.inputPanel.node.active = false;
        _self.aboutPanel.node.active = false;
        _self.helpPanel.node.active = false;
        _self.infoPanel.node.active = false;
        _self.mask.node.active = false;
      },
      doUpdateRoomString: function doUpdateRoomString() {
        var _self = this;
        _self["roomLable1"].string = "";
        _self["roomLable2"].string = "";
        _self["roomLable3"].string = "";
        _self["roomLable4"].string = "";
        for (var index = 0; index < _self.roomString.length; index++) _self["roomLable" + (index + 1)].string = _self.roomString[index];
      },
      doDeleteNum: function doDeleteNum() {
        var _self = this;
        _self.roomString.pop();
        _self.doUpdateRoomString();
      },
      doEnterNum: function doEnterNum(e, num) {
        var _self = this;
        if (4 == _self.roomString.length) return;
        _self.roomString.push(num);
        _self.doUpdateRoomString();
      },
      createRoom: function createRoom() {
        var _self = this, roomInfo = new mvs.MsCreateRoomInfo(), result = null;
        roomInfo.name = "roomName";
        roomInfo.maxPlayer = 2;
        roomInfo.mode = 0;
        roomInfo.canWatch = 2;
        roomInfo.visibility = 0;
        _self.load();
        mvsApp.engine.createRoom(roomInfo, mvsApp.userInfo);
      }
    });
    cc._RF.pop();
  }, {
  ...
  loader: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "e7a4fnyjopKAYn/bpDha7E3", "loader");
    "use strict";
    cc.Class({
      extends: cc.Component,
      properties: {
      ...
      onLoad: function onLoad() {
        var _self = this;
        _self.loader1.runAction(this.actionLoaderLeft());
        _self.loader3.runAction(this.actionLoaderRight());
      },
      actionLoaderLeft: function actionLoaderLeft() {
        var goRight = cc.moveTo(1, cc.v2(-80, 0)).easing(cc.easeCubicActionOut());
        var goLeft = cc.moveTo(1, cc.v2(80, 0)).easing(cc.easeCubicActionIn());
        return cc.repeatForever(cc.sequence(goRight, goLeft));
      },
      actionLoaderRight: function actionLoaderRight() {
        var goRight = cc.moveTo(1, cc.v2(-80, 0)).easing(cc.easeCubicActionIn());
        var goLeft = cc.moveTo(1, cc.v2(80, 0)).easing(cc.easeCubicActionOut());
        return cc.repeatForever(cc.sequence(goLeft, goRight));
      },
      destory: function destory() {
        var _self = this;
        _self.loader1.stopAction(this.actionLoaderLeft());
        _self.loader3.stopAction(this.actionLoaderRight());
      }
    });
    cc._RF.pop();
  }, {} ],
  loading: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "1d6bdJqZBtLYIbnDuaFTJ5N", "loading");
    "use strict";
    var mvsApp = require("./matchvs");
    cc.Class({
      extends: cc.Component,
      ...
      initMatchvs: function initMatchvs() {
        var _self = this;
        mvsApp.init();
        mvsApp.onEvent("login", function(userInfo) {
          window.wx ? wx.login({
            success: function success() {
              wx.authorize({
                ...
                success: function success() {
                  wx.getUserInfo({
                    ...
                    success: function success(res) {
                      ...
                      mvsApp.userInfo.avatar = userInfo.avatarUrl;
                      mvsApp.userInfo.name = userInfo.nickName;
                      cc.loader.load({
                ...
                fail: function fail() {
                  cc.loader.load(userInfo.avatar, function() {
                    cc.director.loadScene("index");
                  });
                }
              });
  ...
  matchvs: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "85e43qDL5dMtK4lRxKhpMoA", "matchvs");
    "use strict";
    var mvs = require("matchvs.all");
    var app = {
      engine: null,
      ...
      init: function init() {
        var _self = this;
        _self.engine = new mvs.MatchvsEngine();
        _self.rsp = new mvs.MatchvsResponse();
        _self.rsp.initResponse = _self.connectResponse.bind(this);
        _self.rsp.registerUserResponse = _self.registResponse.bind(this);
        _self.rsp.loginResponse = _self.loginResponse.bind(this);
        _self.rsp.sendEventNotify = _self.sendEventNotify.bind(this);
      },
      connect: function connect() {
        var _self = this;
        _self.engine.init(_self.rsp, "Matchvs", "alpha", _self.gameID);
      },
      login: function login(id, token) {
        var _self = this;
        var appkey = "fe552ce11aac49f99c1f78753377d3d7#M";
        var secret = "efdec201f09b41d3a161c5bf5e59d018";
        var DeviceID = "abcdef";
        var gatewayID = 0;
        var gameVersion = 1;
        _self.engine.login(id, token, _self.gameID, gameVersion, appkey, secret, DeviceID, gatewayID);
      },
      connectResponse: function connectResponse(status) {
        var _self = this;
        if (200 === status) {
          var rep = _self.engine.registerUser();
          rep && (rep.onDisConnect = function() {
            cc.director.loadScene("loading");
          });
        } else _self.status = -1;
      },
      registResponse: function registResponse(userInfo) {
        var _self = this;
        if (0 === userInfo.status) {
          _self.userInfo = userInfo;
          _self.login(userInfo.id, userInfo.token);
        } else _self.status = -1;
      },
      loginResponse: function loginResponse(rsp) {
        var _self = this;
        if (200 === rsp.status) {
          _self.status = 1;
          _self.emitEvent("login", _self.userInfo);
        } else _self.status = -1;
      },
      onEvent: function onEvent(type, fn) {
        var _self = this, eventArr = _self.eventMap[type] || (_self.eventMap[type] = []);
        eventArr.length && eventArr.splice(0);
        eventArr.push(fn);
      },
      emitEvent: function emitEvent(type) {
        var _self = this, eventArr = _self.eventMap[type], args = Array.prototype.slice.call(arguments);
        if (eventArr) {
          args.shift();
          for (var index = 0, len = eventArr.length; index < len; index++) eventArr[index].apply(_self, args);
        }
      },
      sendEventNotify: function sendEventNotify(eventInfo) {
        var _self = this;
        _self.emitEvent("notify", eventInfo);
      },
      send: function send(msg) {
        var _self = this;
        _self.engine.sendEvent(JSON.stringify(msg));
      },
      disConnectResponse: function disConnectResponse() {
        var _self = this;
        _self.connect();
      }
    };
  ...
  place: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "ec9d6VMPbZLU4eLFJjyY8l6", "place");
    "use strict";
    cc.Class({
      extends: cc.Component,
      properties: {
  ...
  role: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "92087egQGVPY5d8KqXqbwTH", "role");
    "use strict";
    var mvsApp = require("./matchvs");
    cc.Class({
      extends: cc.Component,
      ...
      reduceLife: function reduceLife(val) {
        var _self = this;
        val < 0 && (val = 0);
        _self.life -= val;
        _self.life <= 0 && _self.node.destroy();
      },
      update: function update(dt) {
        var _self = this;
        _self.timer += dt;
        _self.timer > 1 && (_self.status = 0);
        if (_self.status) return;
        "attack" == _self.relation ? _self.node.y -= _self.speed : _self.node.y += _self.speed;
      },
      onCollisionEnter: function onCollisionEnter(other, self) {
        var _self = this, attackRole = other.getComponent("role");
        if (attackRole && attackRole.relation !== _self.relation) {
          _self.beforeAttack = _self.attack;
          _self.status = 1;
          _self.attackMAp[attackRole.id] = 1;
          _self.attack -= attackRole.life < 0 ? 0 : attackRole.life;
          _self.reduceLife(attackRole.beforeAttack || attackRole.attack);
          _self.timer = 0;
          "self" == _self.relation ? _self.node.runAction(cc.moveBy(1, cc.v2(0, -20)).easing(cc.easeCubicActionOut())) : _self.node.runAction(cc.moveBy(1, cc.v2(0, 20)).easing(cc.easeCubicActionOut()));
        }
      },
      onCollisionStay: function onCollisionStay(other, self) {
        var _self = this, attackRole = other.getComponent("role");
  ...
  town: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "66bd7TzdWNCCbOnyqW/8eGi", "town");
    "use strict";
    cc.Class({
      extends: cc.Component,
      properties: {
      ...
      onCollisionEnter: function onCollisionEnter(other, self) {
        var _self = this, role = other.getComponent("role"), attack = 0;
        if (role && role.relation != _self.relation) {
          attack = role.attack;
          other.node.destroy();
          -1 == attack && (attack = 10);
          _self.life -= attack;
          _self.label.string = _self.life;
          _self.lifeBar.progress = _self.life / 100;
          if (_self.life <= 0) {
            var scene = _self.scene.getComponent("game");
            "self" == _self.relation ? scene.lose() : scene.win();
          }
        }
      }
    });
...
}, {}, [ "card", "loader", "game", "index", "loading", "matchvs", "place", "role", "town", "HttpHelper", "matchvs.all" ]);