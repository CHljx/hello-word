__require = function e(t, n, r) {
    ...
    cc.Class({
      ...
      endMove: function endMove() {
        ...
        if (_self.placeTarget) {
          ...
          var nodePosition = _self.node.convertToNodeSpaceAR(worldPosition);
          _self.node.x += nodePosition.x;
          _self.node.y += nodePosition.y;
          _self.status = 2;
      ...
      setOpacity: function setOpacity() {
        ...
        if (0 == _self.status || 2 == _self.status) {
          _self.node.opacity = 255;
          return;
        }
      ...
      update: function update() {
        var _self = this;
        _self.setOpacity();
        2 == _self.status && ("attack" == _self.relation ? _self.node.y -= _self.speed : _self.node.y += _self.speed);
      },
      onCollisionEnter: function onCollisionEnter(other, self) {
        var _self = this, attackCard = other.getComponent("card");
        if (attackCard && attackCard.relation !== _self.relation && 2 == _self.status && 2 == attackCard.status) {
          if (_self.attackMAp[attackCard.id]) return;
          _self.attackMAp[attackCard.id] = 1;
          _self.reduceLife(attackCard.attack);
        }
      },
  ...
  game: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onLoad: function onLoad() {
        ...
        mvsApp.onEvent("notify", _self.notify.bind(_self));
        if (mvsApp.roomOwner) {
      ...
      notify: function notify(eventInfo) {
        ...
        switch (msg.type) {
         case "joinRoom":
          _self.joinRoomNotify(msg.userInfo);
          break;
      ...
      cardNotify: function cardNotify(card) {
        var _self = this;
        _self.addAttackCard(card.index, _self[card.classify]);
      },
      joinRoomNotify: function joinRoomNotify(userInfo) {
        var _self = this;
      ...
      startGame: function startGame() {
        ...
        _self.btnStart.node.active = false;
        _self.btnReady.node.active = false;
        _self.addCard(0, cc.instantiate(_self.card1));
        _self.addCard(1, cc.instantiate(_self.card2));
        _self.addCard(2, cc.instantiate(_self.card3));
        _self.addNext();
        _self.engenTimer = 0;
      },
      ...
      addAttackCard: function addAttackCard(index, component) {
        var _self = this;
        var card = cc.instantiate(component);
        this.node.addChild(card);
        card.setPosition(_self.oppositePos[index]);
        card.getComponent("card").scene = _self;
        card.getComponent("card").status = 2;
        card.getComponent("card").relation = "attack";
      },
      showAttack: function showAttack() {
        var _self = this;
        var attackObj = [];
        var randomLen = 2 * Math.random();
        for (var index = 0; index < randomLen; index++) attackObj.push({
          index: Math.floor(3 * Math.random()),
          card: [ "card1", "card2", "card3" ][Math.floor(3 * Math.random())]
        });
        attackObj.forEach(function(item) {
          _self.addAttackCard(item.index, _self[item.card]);
        });
      },
      ...
      addNext: function addNext() {
        var _self = this;
        _self.cardNext = cc.instantiate([ _self.card1, _self.card2, _self.card3 ][Math.floor(3 * Math.random())]);
      ...
      update: function update(dt) {
        ...
        if (2 == _self.status) {
          _self.engenTimer += dt;
          _self.engenTimer >= 1 && _self.addEnegy();
        }
      }
    });
    cc._RF.pop();
  ...
  index: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      createRoomResponse: function createRoomResponse(rsp) {
        ...
        } else console.log("\u521b\u5efa\u6307\u5b9a\u7c7b\u578b\u623f\u95f4\u63a5\u53e3\u5931\u8d25 status" + rsp.status);
      },
      ...
      inputRoom: function inputRoom() {
        var _self = this;
        _self.inputPanel.node.active = true;
        _self.mask.node.active = true;
      },
      joinRoom: function joinRoom() {
        var _self = this;
        mvsApp.engine.joinRoom(_self.input.string, mvsApp.userInfo);
      },
  ...
  matchvs: [ function(require, module, exports) {
    ...
    var app = {
      ...
      connectResponse: function connectResponse(status) {
        var _self = this;
        200 === status ? _self.engine.registerUser() : _self.status = -1;
      },
  ...
  town: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onCollisionEnter: function onCollisionEnter(other, self) {
        var _self = this, card = other.getComponent("card"), attack = 0;
        if (card && 2 == card.status) {
          attack = card.attack;
          other.node.destroy();
          _self.life -= attack;
          _self.lifeBar.progress = _self.life / 10;
          if (_self.life <= 0) {
            console.log(("self" == _self.relation ? "\u6211\u65b9" : "\u654c\u65b9") + "\u57ce\u5821\u88ab\u6467\u6bc1");
            cc.director.loadScene("game");
          }
        }
      }
...
}, {}, [ "card", "game", "index", "loading", "matchvs", "place", "town", "matchvs.all" ]);